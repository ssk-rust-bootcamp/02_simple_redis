...
use crate::{cmd::CommandError, Backend, RespArray, RespFrame, RespMap, RespNull};
    ...
    fn try_from(value: RespArray) -> Result<Self, Self::Error> {
        ...
        match (args.next(), args.next()) {
            ...
            _ => Err(CommandError::InvalidArgument(
                "Invalid key for HGET command".to_string(),
            )),
        }
    }
}
impl TryFrom<RespArray> for HSet {
    ...
    fn try_from(value: RespArray) -> Result<Self, Self::Error> {
        ...
        match (args.next(), args.next(), args.next()) {
            ...
            _ => Err(CommandError::InvalidArgument(
                "Invalid arguments for HSET command".to_string(),
            )),
        }
    }
}
impl TryFrom<RespArray> for HGetAll {
    ...
    fn try_from(value: RespArray) -> Result<Self, Self::Error> {
        ...
        match args.next() {
            ...
            _ => Err(CommandError::InvalidArgument(
                "Invalid arguments for HGETALL command".to_string(),
            )),
        }
    }
}
...
mod tests {
    ...
    fn test_hset_hget_hgetall_command() -> Result<()> {
        ...
        let cmd = HGetAll { key: "map".to_string() };
        let result = cmd.execute(&backend);
        let mut expected = RespMap::new();
        expected.insert("hello".to_string(), RespFrame::BulkString(b"world".into()));
        // expected.insert("hello1".to_string(), RespFrame::BulkString(b"world1".into()));
